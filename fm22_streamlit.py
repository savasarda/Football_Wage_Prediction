...
with row0_1:
    st.markdown(f'<h1 style="color:#FF0000;font-size:40px;">{"Football Players Wage Prediction"}</h1>', unsafe_allow_html=True)
row3_spacer1, row3_1, row3_spacer2 = st.columns((.1, 3.2, .1))
with row3_1:
    st.markdown(f'<h1 style="color:#FF0000;font-size:25px;">{"Arda Savaş - Emin Akdemir"}</h1>', unsafe_allow_html=True)
...
with row3_1:
    st.markdown("")
    see_data = st.expander('All Data Values')
    with see_data:
...
source = html.read()
components.html(source,width=1750,height=500)
